...
# Add project root (2 levels up: streamlit/ → frontends/ → project root)
_proj_root = os.path.dirname(os.path.dirname(script_dir))
for _p in (_proj_root, os.path.abspath(os.path.join(script_dir, '..')), os.path.abspath(script_dir)):
...
import frontends.shared.chatapp_common as chatapp_common  # activate /continue command (monkey patches GeneraticAgent)
from frontends.cmd.continue_cmd import handle_frontend_command, reset_conversation, list_sessions, extract_ui_messages
from frontends.cmd.btw_cmd import handle_frontend_command as btw_handle_frontend
...
def render_sidebar():
    ...
    if st.button(T('desktop_pet')):
        ...
        if not os.path.exists(pet_script): pet_script = os.path.join(script_dir, '..', 'desktop', 'desktop_pet.pyw')
