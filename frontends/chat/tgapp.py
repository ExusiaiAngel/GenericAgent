...
from frontends.cmd.continue_cmd import handle_frontend_command, reset_conversation
from frontends.cmd.btw_cmd import handle_frontend_command as handle_btw_frontend_command
from frontends.cmd.review_cmd import handle as handle_review_command
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            # 代理隧道链路下放宽读写超时：短超时会把 LLM 已完成的回复
            # 在发送阶段误判为失败（[TG stream error] TimedOut）。
            request_kwargs = dict(read_timeout=90, write_timeout=90, connect_timeout=30, pool_timeout=30)
            if proxy:
