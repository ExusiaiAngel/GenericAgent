...
Goal of this module: keep the TUI file (tuiapp_v2.py) thin. It only
needs to forward `/update`, `/autorun`, `/morphling`, `/goal`, `/hive`
...
def detect_user_shell() -> tuple[list[str], str]:
    ...
    `!cmd` in tuiapp_v2.py invokes this so commands like `ls`, pipes,
...
#   1. `GA_LANG` env var (scriptable override)
#   2. persisted settings file (same path as the TUI's settings)
#   3. system locale (zh* → 'zh', else 'en')
# When the user switches language inside the TUI (set_lang persists), the next
...
def build_update_prompt(args_text: str = "") -> str:
    ...
    Language follows `_current_lang()` so a /language switch in the TUI (or a
...
_HUB_EXCLUDES = {"goal_mode.py", "chatapp_common.py", "tuiapp.py", "stapp2.py"}
...
# (cmd, arg_hint, desc)  — slash-command palette for the TUI frontend.
