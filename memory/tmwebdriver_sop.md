...
- 底层：`../TMWebDriver.py`通过Chrome/Edge扩展接管用户浏览器（保留登录态/Cookie）
...
## 浏览器后台标签节流（Chrome/Edge）
- 后台标签中`setTimeout`被浏览器 intensive throttling 延迟到≥1min/次，扩展脚本中避免依赖setTimeout轮询（Chrome/Edge行为一致）
...
- contentSettings：`{cmd:'contentSettings', type:'automaticDownloads', pattern:'https://*/*', setting:'allow'}`
  - 绕过浏览器"下载多个文件"对话框（该对话框会阻塞整个浏览器JS执行）
...
- ⭐**坐标系结论**：稳定状态下 CDP坐标 = `getBoundingClientRect()` 坐标，**无需修正**
  - ⚠**首次attach陷阱**：CDP debugger首次attach时浏览器弹出infobar("正在受自动化控制"，~20px高，Chrome/Edge均有此行为)，页面内容被推下
...
## autofill获取与登录
检测：web_scan输出input带`data-autofilled="true"`，value显示为受保护提示(非真实值，浏览器安全保护需点击释放)
- ⚠**前置条件：必须先CDP `Page.bringToFront` 切tab到前台**，浏览器仅在前台tab释放autofill保护值，后台tab物理点击无效
