...
2.  **神圣不可删改性 (Sanctity of Verified Data)**
    ...
    *   记忆修改时请极度小心。已有内容只能少量 `file_patch`；禁止 overwrite、append/prepend 和 `code_run`。
...
- 已完成的 `long_task` 会触发一次最多 4 轮的隐藏 memory-only 结算；失败、待确认、停止、超轮和短聊天不触发。
- 自动结算只能依据刚完成任务中的工具结果，且只能最小 patch 已有 L1/L2/顶层 L3 文本，或新建一个顶层 L3 Markdown。
- 自动结算输出不进入用户回复或聊天历史；没有合格信息时必须 `NO_MEMORY`，不得为了“有更新”而写入。
- 密钥、临时状态、源码、JSON、嵌套目录、软链接和 L4 原始会话在技术层保持禁止。
```
...
- 工具脚本（*.py）：仅封装高复用、逻辑相对复杂且不希望每次都重新推理的处理流程；Generic 自身的受控记忆能力不得修改或新建脚本，需由用户/管理员审查部署。

### 受控写入协议

- 除非用户当前消息已明确批准本次记忆更新，否则先 `ask_user`，说明准确路径、动作、原因和验证方式。
- 已有顶层 `.md`、`global_mem.txt`、`global_mem_insight.txt`：只用 `file_patch`。
- 新 L3：只允许 `file_write` 新建顶层 `.md`；若文件已存在则必须改用 `file_patch`。
- 禁止：覆盖/追加/前插已有记忆、嵌套路径、符号链接逃逸、`.py`/JSON/备份/L4 修改、通过 `code_run` 绕过。
---
## L1 ↔ L2/L3 同步规则
| 操作 | L1 同步 |
|---------|--------|
...
- L1：最多 30 行且最多 4096 bytes。
- L2：最多 16384 bytes；超限前必须合并或删除失效条目。
- 启动 Profile：最多 8192 bytes。
- 单个顶层 L3 Markdown：最多 16384 bytes。
- 单次隐藏自动结算净增长：最多 1024 bytes。
- 超限写入必须失败并返回 `consolidate before writing`，禁止截断后伪装成功。
- L4 清理只能由保留策略生成候选，默认 dry-run；不得由普通模型工具删除。

## Skill 提案与审批

- 可复用流程先用 `skill_propose` 写入受保护的待审批区，不得直接成为活动 Skill。
- 审批绑定 `platform + account + conversation + actor`，且绑定提案 SHA-256；不同平台、会话或操作者不能代批。
- 当前只允许安装 Markdown `SKILL.md`；脚本、模板、修改和删除继续禁止。
- 活动 Skill 位于 `memory/skills/<slug>/SKILL.md`，仅在相关任务中按需读取。
...
| v2 | 2026-07-13 | 增加硬预算、L4 dry-run 保留策略和 Skill 提案审批协议 |
