...
- **Sandbox subdirectories:**
  ...
  - `sandbox\trash_review`: files proposed for deletion or replacement; never delete originals without confirmation.
- **TMWebDriver CDP Bridge:** Installed in Edge (Profile: Exusiai), path: `assets/tmwd_cdp_bridge/`
- **WSL legacy:** Previous workspace at `\\wsl.localhost\Ubuntu-24.04\home\exusiai\GenericAgent`, no longer active. `env.sh` retains WSL paths for Git Bash fallback.
...
- During the cold-start supervision phase, prefer read-only observation of real projects and write only inside `D:\GenericAgent\sandbox` unless the user explicitly approves another path.
...
- Do not modify files outside `D:\GenericAgent\sandbox` unless the user explicitly approves the exact path and action.
