...
def discover_services():
    services = []
    EXCLUDES = {'goal_mode.py', 'chatapp_common.py', 'stapp2.py'}
    ...
    if os.path.isdir(frontends_dir):
        for root, dirs, files in sorted(os.walk(frontends_dir)):
            # skip __pycache__ and non-app subdirs
            dirs[:] = [d for d in dirs if not d.startswith('__') and d not in ('skins', 'temp')]
            ...
            for f in sorted(files):
                if 'app' in f and f.endswith('.py') and f not in EXCLUDES:
                    ...
                    if 'stapp' in f: cmd = [sys.executable, '-m', 'streamlit', 'run', rel_path, '--server.headless=true']
                    else: cmd = [sys.executable, rel_path]
                    services.append({'name': rel_path, 'cmd': cmd})
    return services
